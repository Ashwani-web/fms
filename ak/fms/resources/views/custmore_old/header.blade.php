				...
				<div class="row">
					...
					<div class="collapse navbar-collapse" id="navbarSupportedContent">
						<ul class="navbar-nav m-auto">
							<li class="nav-item has-dropdown-2 drop-btn">
								<a class="nav-link drop-for-mob drop-btn-mob" href="javascript:void(0)">{{$furSubMenu->name}}</a>
									...
									@if(!empty($furSubMenu->sub_category))
										@foreach($furSubMenu->sub_category as $subCategoryName)
											...
											<li class="nav-item  has-dropdown-2 sub-drop-btn">
												<a class="nav-link drop-for-mob sub-drop-btn-mob" href="#">{{$subCategoryName->subCatname}}</a>
														...
														<a href="javascript:void(0)" class="back-btn-sub-sub d-lg-none nav-link"> &lt; Back to main menu</a>
																...
																@if(!empty($subCategoryName->fur_sub_category))
																	@foreach($subCategoryName->fur_sub_category as $furCategoryName)
																		...
																		<li class="nav-item">
																			<a class="nav-link" href="#">{{$furCategoryName->furName}}</a>
																		</li>
																@endif
																	@endforeach
																		@endif
					...
					<div class="collapse navbar-collapse" id="navbarSupportedContent">
						...
						<ul class="dropdown-nav">
								...
								<li class="nav-item has-dropdown-2 sub-drop-btn">
										<a class="nav-link drop-for-mob sub-drop-btn-mob" href="#">{{$furSubMenus->name}}</a>
										<ul class="sub-dropdown-nav d-none">
										@if(!empty($furSubMenus->sub_category))
											...
											<li class="nav-item">
												<a class="nav-link" href="#">{{$sublastCategoryName->subCatname}}</a>
											</li>
											@endif
										    	@endforeach
													@endif
